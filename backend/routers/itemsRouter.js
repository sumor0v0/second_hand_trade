...
router.post("/", authMiddleware, async (req, res, next) => {
  try {
    const { categoryId, title, description, price, status } = req.body || {};

    if (!categoryId || !title || !price) {
      return res
        .status(400)
        .json({ message: "categoryId, title and price are required" });
    }
    ...
    const [rows] = await db.query(
      `SELECT i.id, i.title, i.description, i.price, i.status, i.created_at, i.category_id,
...
router.get("/", async (req, res, next) => {
  try {
    const { category, keyword, minPrice, maxPrice, sellerId, status } =
      req.query || {};
    ...
    const [rows] = await db.query(
      `SELECT i.id, i.title, i.description, i.price, i.status, i.created_at,
              u.username AS seller_name, i.category_id,
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT i.id, i.title, i.description, i.price, i.status, i.created_at,
              i.category_id, i.seller_id, u.username AS seller_name
         FROM items i
         JOIN users u ON u.id = i.seller_id
        WHERE i.id = ?`,
      [itemId]
